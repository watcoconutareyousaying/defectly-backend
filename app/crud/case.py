...
def create_case(db: Session, requirement_id: int, creator_id: int, case_data: Dict[str, Any]):
    ...
    tc = Case(requirement_id=requirement_id,
              created_by=creator_id, case_data=case_data)
    db.add(tc)
    db.commit()
    db.refresh(tc)
    return tc
...
def get_cases_for_project(
    ...
    if status:
        q = q.filter(
            func.lower(
                text("(case_data->>'status')")  # PostgreSQL JSON text extraction
            ) == status.lower()
        )
    ...
    if search:
        ...
        q = q.filter(
            or_(
                func.lower(cast(Case.case_data['title'], String)).like(
                    search_pattern),
                func.lower(cast(Case.case_data['description'], String)).like(
                    search_pattern)
            )
        )
